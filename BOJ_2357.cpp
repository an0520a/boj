...
class ops{public : constexpr entry operator()(const entry& x, const entry& y){ return {(x.min < y.min)? x.min : y.min, (x.max > y.max)? x.max : y.max};}};
...
int main()
{
    ...
    for (u32 i = 0; i < N; i++)
    {
        ...
        arr[i].min = arr[i].max = tmp;
    }
    SegmentTree<entry, ops> tree(arr, N);
