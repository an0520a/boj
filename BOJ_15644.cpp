...
void DFS(const MarbleMap& map, u32 stage, u32 count, char* storage);
...
void DFS(const MarbleMap& map, u32 stage, u32 count, char* storage)
{
    ...
    else
    {
        ...
        val = tmp.up();
        storage[stage] = 'U';
        if     (val ==  1 && stage < min - 1) DFS(tmp, stage + 1, count - 1, storage);
        else if(val == -1)                    { min = (stage + 1 < min)? stage + 1 : min; memcpy(memo, storage, (stage + 2) * sizeof(char)); storage[stage] = '\0'; return; }
