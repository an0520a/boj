...
{
public:
    u32 operator()(const u32& i, const u32& j) const { return (arrG[i] <= arrG[j])? i : j; }    
};
...
int main()
{
    ...
    scanf(" %u", &N);
    u32* arrIdx = new u32[N];

    for (u32 i = 0; i < N; i++) scanf(" %u", arrG + i);
    for (u32 i = 0; i < N; i++) arrIdx[i] = i;

    SegmentTree<u32, MIN> tree(arrIdx, N);
    ...
    for (u32 T = 0; T < M; T++)
    {
        ...
        if(a == 1) arrG[i - 1] = j, tree.change(i - 1, i - 1);
        else       printf("%u\n", tree.getVal(i - 1, j - 1) + 1);
    }
}