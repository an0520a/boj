...
public:
    ...
    {
        printf("tree-----------\n");
        for (size_t i = 0; i < size; i++) printf("%llu\n", tree[i]);
        printf("\n");
        printf("arr------------\n");
        for (size_t i = 0; i < sizeArr; i++) printf("%llu\n", arr[i]);
        printf("\n");
    }
...
constexpr u32 MUL(const u32 &x, const u32 &y) { return (u32)(((u64)x * (u64)y) % 1000000007ULL); }
...
int main()
{
    ...
    for (u32 i = 0; i < N; i++) scanf(" %llu", arr + i);

    SegmentTree<u32, decltype(&MUL)> tree(arr, N, MUL);
